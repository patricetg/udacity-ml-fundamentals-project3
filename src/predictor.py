...
class Predictor(nn.Module):
    ...
    def __init__(self, model, class_names, mean, std):
        ...
        self.softmax = nn.Softmax(dim=1) # my own code
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            # 1. apply transforms
            x  = self.transforms(x) # YOUR CODE HERE (done)
            # 2. get the logits
            x  = self.model(x) # YOUR CODE HERE (done)
            # 3. apply softmax
            #    HINT: remmeber to apply softmax across dim=1
            x  = self.softmax(x) # YOUR CODE HERE (done)
