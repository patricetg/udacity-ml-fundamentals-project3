...
def get_model_transfer_learning(model_name="resnet18", n_classes=50):
    ...
    num_ftrs  = model_transfer.fc.in_features # YOUR CODE HERE (done)

    # 2. Create a new linear layer with the appropriate number of inputs and
    #    outputs
    #model_transfer.fc  = nn.Linear(num_ftrs, n_classes) # YOUR CODE HERE (done)
    # YOUR CODE HERE (done)
...
def data_loaders():
    ...
    return get_data_loaders(batch_size=2, num_workers=1) # get_data_loaders(batch_size=2)
