...
def get_data_loaders(
  ...
  batch_size: int = 32, valid_size: float = 0.2, num_workers: int = -1, limit: int = -1
    ...
    # Getting the classes immediatly to make things easier for me in the future
#     classes = train_data.classes
    ...
    split = int(math.floor(valid_size * n_tot)) #int(math.ceil(valid_size * n_tot))
    train_idx, valid_idx = indices[split:], indices[:split]

    # define samplers for obtaining training and validation batches
    train_sampler = torch.utils.data.SubsetRandomSampler(train_idx)
    valid_sampler  = torch.utils.data.SubsetRandomSampler(valid_idx) # YOUR CODE HERE (done)
    ...
    test_data = datasets.ImageFolder(
        base_path / "test",
        # YOUR CODE HERE (add the test transform) (done)
    ...
    data_loaders["test"] = torch.utils.data.DataLoader(
        # YOUR CODE HERE (remember to add shuffle=False as well) (done)
        test_data,
        batch_size=batch_size,
...
def visualize_one_batch(data_loaders, max_n: int = 5):
    ...
    dataiter  = iter(data_loaders.get("train"))  # YOUR CODE HERE (done)
    # Then call the .next() method on the iterator you just
    # obtained
    images, labels  = dataiter.next() # YOUR CODE HERE (done)
    ...
    class_names  = data_loaders.get("train").dataset.classes # YOUR CODE HERE (done)
