...
def train_one_epoch(train_dataloader, model, optimizer, loss):
    ...
    if torch.cuda.is_available():
        ...
        # HINT: use .cuda()
        model = model.cuda() #model.to(torch.float16).cuda() #

    # YOUR CODE HERE: set the model to training mode (done)
    model.train()
    ...
    for batch_idx, (data, target) in tqdm(
        ...
        output  = model(data)  # YOUR CODE HERE (done)
        
        # 3. calculate the loss
        loss_value  = loss(output, target) # YOUR CODE HERE (done)
...
#def optimize(data_loaders, model, optimizer, loss, n_epochs, save_path, interactive_tracking=False, last_epoch_val_loss_path:str="checkpoints/last_epoch_val_loss.pt",last_epoch_info_path:str= "checkpoints/last_epoch_info.json"):
...
#         # my own adding: saving the model at the end of each epoch, so that training could resume from here
#         if last_epoch_val_loss_path is not None:
#             torch.save(model.state_dict(), last_epoch_val_loss_path)
            
#             dictionary = {"epoch":epoch}
#             with open(last_epoch_info_path, "w") as outfile:
...
def one_epoch_test(test_dataloader, model, loss):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data, target) in tqdm(
            ...
            # 1. forward pass: compute predicted outputs by passing inputs to the model
            logits  = model(data) # YOUR CODE HERE (done)
            
            # 2. calculate the loss
            loss_value  = loss(logits, target) # YOUR CODE HERE (done)
            ...
            pred  = logits.data.max(1, keepdim=True)[1]  # YOUR CODE HERE (done)
